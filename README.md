...
Запасной путь (уже не AC-DC): мост + LM2596HV.
...
| готовый 24VAC→5V | AC 8–30V | 2–5A | мост не нужен |

Не аналоги под 25V~: XL4015 (до 38V), MP1584 (до 28V), Mini360 (до 23V), Mini560 (до 32V), MT3608 и XL6009 (boost вверх).
 (вход 4.5–60V DC). После 25V~ ≈ 33–35V=, HV хватает. Выход выставить 5.0–5.2V до Pi, ток реально ~1.5–2A с радиатором.
...
Опционально позже: второй канал реле, датчик почвы, солнечная панель 6V + контроллер вместо USB на TP4056.
...
2. На boost мультиметром выставь **5.0–5.2V на холостом ходу**, до подключения Pi.
...
2. На холостую измерить выход: **5.0–5.2V**. Если есть подстроечник — выставить.
...
Расписание без system cron и без `node-cron`: дни недели (или «все»), время и длительность. Несколько строк (например будни 07:00 и выходные 09:00). Старое поле `cron_expr` один раз импортируется в таблицу `schedules` и больше не восстанавливается после удаления всех строк.
